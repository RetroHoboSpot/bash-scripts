...
# - Teams notification with per-stack results (CSV-based)
# Options:
...
json_escape_multiline() {
  # Escapes a multi-line string for safe JSON embedding (quotes, backslashes, newlines)
...
build_stack_results_markdown() {
  # Builds per-stack results from CSV: timestamp,project,dir,status,details
  # Output is Markdown lines suitable for Teams MessageCard "text".
  if [[ ! -f "$REPORT_CSV" ]]; then
    echo "ℹ️ No CSV report found."
    return 0
  fi
  ...
  awk -F',' '
    NR==1 { next }  # skip header
    {
      ...
      status=$4
      icon="ℹ️"
      if (status=="UPDATED") icon="✅"
      else if (status=="NO_UPDATES") icon="⏭"
      else if (status=="SKIPPED") icon="⏭"
      else if (status=="DRY_RUN") icon="🧪"
...
send_teams_notification() {
  # Uses Teams Incoming Webhook (MessageCard)
  # Only sends if TEAMS_WEBHOOK_URL is set
  [[ -n "$TEAMS_WEBHOOK_URL" ]] || return 0
  command -v curl >/dev/null || { echo "WARNING: curl not found; cannot send Teams notification"; return 0; }

  local STATUS="$1"
  local COLOR="$2"

  local MODE_TEXT
  MODE_TEXT="$([ "$DRY_RUN" = true ] && echo "DRY-RUN" || echo "LIVE")"

  local STACK_MD RAW_TEXT STACK_ESC
  STACK_MD="$(build_stack_results_markdown)"
  RAW_TEXT="**Totals:** Total=$TOTAL | Updated=$UPDATED | NoUpdates=$NO_UPDATES | Skipped=$SKIPPED | DryRun=$DRYRUNS | Errors=$ERRORS\n\n**Per-stack results:**\n\n${STACK_MD}\n\nReport: ${REPORT_TXT}"

  STACK_ESC="$(printf "%b" "$RAW_TEXT" | json_escape_multiline)"

  curl -fsSL -X POST "$TEAMS_WEBHOOK_URL" \
    -H "Content-Type: application/json" \
    -d @- <<EOF
{
  "@type": "MessageCard",
  "@context": "http://schema.org/extensions",
  "summary": "Docker Compose Update",
  "themeColor": "$COLOR",
  "title": "Docker Compose Update – $(hostname)",
  "sections": [
    {
      "facts": [
        { "name": "Status", "value": "$STATUS" },
        { "name": "Mode", "value": "$MODE_TEXT" },
        { "name": "Time", "value": "$(date '+%Y-%m-%d %H:%M')" }
      ]
    },
    {
      "text": "$STACK_ESC"
...
# ----------------------------------------------------------
# Teams notification (end-of-run)
# ----------------------------------------------------------
if [[ -n "$TEAMS_WEBHOOK_URL" ]]; then
  if (( ERRORS > 0 )); then
    send_teams_notification "Errors detected" "D32F2F"
  elif (( UPDATED > 0 )); then
    send_teams_notification "Updates applied" "00C853"
  elif (( DRYRUNS > 0 )); then
    send_teams_notification "Dry-run complete" "1976D2"
  else
    send_teams_notification "No updates" "607D8B"
  fi
fi
